export function parser(tokens) {
	...
	function take(type, mode) {
		if (Array.isArray(type)) {
			const index = type.findIndex(c => c === token.type);
	...
	function FunctionStatement() {
		...
		const body = Block();
		if (!body) panic('Expected a Bloc for the function');
	...
	function VariableStatement() {
		const kw = maybeTake('VariableDeclaration');
		if (!kw) return null;

		const name = take('Id');
		take('Assignment');
		const value = take(['String', 'Numeric', 'Boolean'], 'expression');
